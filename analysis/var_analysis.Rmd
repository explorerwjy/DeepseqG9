...
levels1 %>%
  mutate(score = as.numeric(score)) %>%
  ggplot(aes(score, ..density.., fill = reference)) +
  geom_histogram(alpha = 0.7, position = "identity")+
  scale_fill_manual(values=c("deepskyblue", "coral1")) +
  ggtitle("DeepSeq9 Logistic Histogram") 

levels1 %>%
  group_by(reference, prediction) %>%
  count()

levels %>% filter(is.na(score)) %>% group_by(reference, prediction) %>% count()
```
...
rf.prob1 = rf.prob 
rf.prob$pred[rf.prob$pred > 0.5] <- "P"
rf.prob$pred[rf.prob$pred <= 0.5] <- "B"

#class is reference
#pred is prediction
cro(rf.prob$pred, data_test$class)
...
ggroc(roc.rf) +
  scale_color_manual(labels = "DeepSeq9",
                     values = "red") +
  geom_abline(intercept = 1, slope = 1, color="darkslategrey", 
                 linetype="dashed", size= 0.5) +
...
#class is reference
#class1 is prediction
rf.prob1$class1 <- rep("B", length(rf.prob1))
rf.prob1$class1[rf.prob1$score >= 0.1290769 & rf.prob1$score <  0.2283199] <- "LB"
rf.prob1$class1[rf.prob1$score >= 0.2283199 & rf.prob1$score <  0.8237779] <- "VUS"
rf.prob1$class1[rf.prob1$score >= 0.8237779 & rf.prob1$score <  0.9230209] <- "LP"
rf.prob1$class1[rf.prob1$score > 0.9230209] <- "P"

cro(rf.prob1$class1, data_test$class)
