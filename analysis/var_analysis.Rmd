...
plot(roc.glm, col = 1, main = "ROC")
plot(roc.cadd, col = 2, add = TRUE)
plot(roc.msvm, col = 3, add = TRUE)
plot(roc.sift, col = 4, add = TRUE)
plot(roc.poly, col = 5, add = TRUE)
...
subset = data[sample(nrow(data), 285820), ]  #25% 
train <- sample(1:nrow(subset), nrow(subset)/2)


#random forest

rf.var <- randomForest(class ~ ., subset[train,], mtry=5, importance =TRUE, na.action = na.exclude)

importance(rf.var)

pred.test.rf <- predict(rf.var, newdata = subset[-train,], type = "prob")

table(subset[-train]$class, pred.test.rf)
