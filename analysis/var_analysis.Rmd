...
levels$prediction[levels$score > 0.1 & levels$score < 0.9] <- "U"
levels$prediction[levels$score > 0.9] <- "P"
...
log_scores = as.data.frame(test.pred.prob) %>%
  rename(prediction_score = test.pred.prob)

log_scores %>%
  ggplot(aes(prediction_score, ..density..)) +
  geom_histogram() +
  ggtitle("Logistic Regression Score Histogram") 
...
rf.var <- randomForest(class ~ ., data2[train,], mtry=5, importance =TRUE, na.action = na.exclude)
rf.var

importance(rf.var)

pred.test.rf <- predict(rf.var, newdata = data2[-train,], type = "prob")


table(pred.test.rf, data2[-train]$class)
